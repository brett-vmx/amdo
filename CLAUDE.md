...
## Cover art sizing
Covers are custom-illustrated (replacing the original placeholder art from another project), 1280×720 WebP, 16:9 to match the actual video files' native resolution (854×480, also 16:9) — this matters because these images double as `<video poster>`: a mismatched aspect ratio letterboxes and can cause a layout jump when the video's real dimensions load. One image per story serves both the grid and the poster — the grid card (`StoryCard.astro`) shows the full image via `aspect-video` (no cropping, changed from an earlier square crop that clipped subjects at the edges on wide compositions). 1280px wide is 2x retina-sharp at the largest CSS display width in the app (~680px, the modal/story-page container) — `getImage()` in both `index.astro` and `[slug].astro` requests `width: 1280` to actually deliver that resolution to the browser (bumped up from an earlier 680, which was capping output regardless of source size).

**Replacing a cover image file:** if you swap a source file in `src/assets/stories/covers/` for a new one with the *same filename* (as opposed to changing the frontmatter path) and the new file happens to share the old one's dimensions/format, the resulting `/_image?...` request URL is byte-identical to before — two independent caches can then serve the stale version even though the source file itself is correctly updated:
1. **Dev server process cache** — Astro's dev `/_image` endpoint memoizes transforms in-memory per process, keyed by request URL. Fix: restart the dev server.
2. **Browser HTTP cache** — the endpoint sends `Cache-Control: public, max-age=31536000` (1 year), so a browser that already loaded that exact URL will keep reusing it even across a hard reload, in a brand-new tab, in the same profile. Fix: append a cache-busting query param to force a real fetch, or just check in a browser/profile that's never hit that URL before.

Confirmed both independently via `/@fs/...` (raw file correct) and a manual cache-busted `_image` request (also correct) — don't assume a stale screenshot means the swap failed. Production builds (`npm run build`) aren't affected by either — each build reprocesses from scratch and gets a fresh content hash.
