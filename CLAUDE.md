...
**Replacing a cover image file:** if you swap a source file in `src/assets/stories/covers/` for a new one with the *same filename* (as opposed to changing the frontmatter path), the Astro **dev server's** `/_image` transform endpoint can keep serving the old cached output even after a hard reload / cache-busted fetch — it caches transforms in-memory per process, keyed by the request URL, which doesn't change if the new file happens to share the old one's dimensions/format. Confirmed via `/@fs/...` that the raw file itself was correct; only the transform cache was stale. Fix: restart the dev server (`npm run dev`). Production builds (`npm run build`) aren't affected — each build reprocesses from scratch.
